...
# Save
```{r}
rowData(dep_ctrl_vs_E_pY) %>% as_tibble() %>% select(HGNC_Symbol, E_vs_ctrl_diff) %>% write.table("../Data/Kinase_enrichment/Batch2_Set4_E_vs_ctrl_pY_forstring.txt", quote = F, row.names = F, col.names = F, sep = "\t")
