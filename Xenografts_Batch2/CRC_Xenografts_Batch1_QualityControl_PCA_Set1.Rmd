...
```{r, fig.width=9, fig.height=3}
PCA_pST <- pST_mat_Set1_oneshot %>%
  prcomp() %>% 
  .$x %>% 
  as.data.frame() %>%
  rownames_to_column("sample")

PCApST_rot <- pST_mat_Set1_oneshot %>%
  prcomp() %>%
  .$rotation
PCApST_rot <- bind_cols(PCApST_rot, 
  (pST_Set1_oneshot_form %>% select(HGNC_Symbol, Annotated_Sequence) ) ) %>% arrange(desc(PC1) )
