---
title: "Quality control PCA - CRC Batch 2 - Set 4"
...
## Nr. phospho sites
```{r}
print( paste( nrow(pY_Set4_form) , "pY peptides passed the filtering procedure for the sets combined. These peptides were detected from", length(unique(pY_Set4_form$HGNC_Symbol) ), "proteins." ))
print( paste( nrow(pST_Set4_form) , "pST peptides passed the filtering procedure for the sets combined. These peptides were detected from", length(unique(pST_Set4_form$HGNC_Symbol) ), "proteins." ))
```
...
PCApY_rot <- pY_mat_Set4 %>%
  prcomp() %>%
  .$rotation
PCApY_rot <- bind_cols(PCApY_rot, 
  (pY_Set4_form %>% select(HGNC_Symbol, Annotated_Sequence) ) ) %>% arrange(desc(PC1) )
...
```{r, fig.width=9, fig.height=3}
PCA_pST <- pST_mat_Set4 %>%
  prcomp() %>% 
  .$x %>% 
  as.data.frame() %>%
  rownames_to_column("sample")

PCApST_rot <- pST_mat_Set4 %>%
  prcomp() %>%
  .$rotation
PCApST_rot <- bind_cols(PCApST_rot, 
  (pST_Set4_form %>% select(HGNC_Symbol, Annotated_Sequence) ) ) %>% arrange(desc(PC1) )
