---
title: "Quality control PCA- CRC Batch 1 Set 2"
...
print( paste( nrow(pST_Set2_form) , "pST peptides passed the filtering procedure for the sets combined. These peptides were detected from", length(unique(pST_Set2_form$HGNC_Symbol) ), "proteins." ))
```

## Principle component analysis Phospho-Tyrosines
...
PCApY_rot <- pY_mat_Set2 %>%
  prcomp() %>%
  .$rotation
PCApY_rot <- bind_cols(PCApY_rot, 
  (pY_Set2_form %>% select(HGNC_Symbol, Annotated_Sequence) ) ) %>% arrange(desc(PC1) )
...
## Principle component analysis Phospho-Serine/Threonine
```{r, fig.width=3, fig.height=3}
pST_mat_Set2 %>%
  prcomp() %>% 
  summary() %>% 
...
```{r, fig.width=9, fig.height=3}
PCA_pST <- pST_mat_Set2 %>%
  prcomp() %>% 
  .$x %>% 
  as.data.frame() %>%
  rownames_to_column("sample")

PCApST_rot <- pST_mat_Set2 %>%
  prcomp() %>%
  .$rotation
PCApST_rot <- bind_cols(PCApST_rot, 
  (pST_Set2_form %>% select(HGNC_Symbol, Annotated_Sequence) ) ) %>% arrange(desc(PC1) )
