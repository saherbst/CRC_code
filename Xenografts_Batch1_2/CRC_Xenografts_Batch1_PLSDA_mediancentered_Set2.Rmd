...
plsda_pY <- Tune_And_Perform_PLSDA(
  X = pY_mat_Set2,
  Y = as.factor( str_split( gsub("log2FC_Xenograft_", "", rownames( pY_mat_Set2 ) ), 
...
##### Plot Components
```{r}
#plsda_pY$variates$X %>% as.data.frame() %>%
#  rownames_to_column("names") %>% as.tibble() %>%
#  separate(names, c("log2FC", "xeno", "treatment", "day", "replicate", "set")) %>%
#  ggplot(aes(comp1, treatment, color = treatment )) + geom_point(size = 3) +
#  scale_color_manual(values = PGPalette[c(5,1,2,4)],) + 
#  theme_bw()

plotIndiv(plsda_pY, comp = c(1,2), # plot samples from final model
          col.per.group = PGPalette[c(5,1,2,4)],
...
##### Plot Loadings
```{r, fig.height=7}
plotLoadings(plsda_pY, method = 'mean', contrib = 'max', comp = 1, size.name = 0.5, size.title = 1, legend.color = PGPalette[c(5,1,4,2)])    
plotLoadings(plsda_pY, method = 'mean', contrib = 'max', comp = 2, size.name = 0.5, size.title = 1, legend.color = PGPalette[c(5,1,4,2)])   
#plotLoadings(plsda_pY, method = 'mean', contrib = 'max', comp = 3, size.name = 0.5, size.title = 1, legend.color = PGPalette[c(5,1,2,4)])  
```
...
Plot_StringDB(
  ...
  #names(plsda_pY$loadings$X[ (plsda_pY$loadings$X[,1] != 0 ),] ) %>% 
