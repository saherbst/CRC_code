...
Return_DEP_Hits_Plots <- function(data, DEP_result, comparison){
  ...
  if(nrow(rowData(DEP_result) %>% 
    ...
    if(nrow(hits) >1){
      ...
      #Minimize_df(name_l = name_l, FC = FALSE)
      rownames_data <- paste0(data$HGNC_Symbol, "_", data$Annotated_Sequence)
      data <- data  %>%  
        select(-HGNC_Symbol, -Annotated_Sequence,           -Master.Protein.Descriptions, -Master.Protein.Accessions) %>%
        as.matrix() 
      colnames(data) <- gsub("log2FC_", "", colnames(data))
...
### Function Barplots
plot_barplot_phos <- function(dat, prot, add_title = ""){
...
plot_barplot_prot <- function(dat, prot, add_title = ""){
  if(prot %in% dat$HGNC_Symbol){
    dat %>%
